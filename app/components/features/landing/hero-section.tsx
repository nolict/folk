...
export const HeroSection = () => {
  ...
  const handleSubmit = () => {
    if (selectedFile && previewUrl) {
      ...
      const encodedType = encodeURIComponent(selectedFile.type);
      router.push(`/editor?preview=${encodedUrl}&type=${encodedType}`);
    }
  };
    ...
    <section className="relative flex flex-col items-center justify-center overflow-hidden bg-white py-24 md:py-32">
      <div className="container mx-auto px-6 text-center">
        ...
        <div className="mt-12 flex flex-col items-center justify-center gap-4 sm:flex-row">
          <Button variant="primary" size="lg" className="w-full sm:w-auto" onClick={handlePickPhoto}>
            Pick Photo or Video
          </Button>
          <input
            ref={fileInputRef}
          ...
          />
          {selectedFile ? (
            <Button variant="primary" size="lg" className="w-full sm:w-auto" onClick={handleSubmit}>
              Submit
            </Button>
          ) : (
